...
- Fixed Tables 1 & 2 disappearing: `desc.tex` and `link_stats.tex` had `\begin{table}` wrappers from `kable_styling()`, causing nested table environments. Removed `kable_styling(latex_options = "hold_position")` from all 7 table outputs across 5 scripts.
- Fixed Table 3 formatting: removed stray `\addlinespace`, added `\midrule` section dividers
- Added kableExtra wrapper rule to global `latex-conventions.md`
- **Next steps**:
