...
- Fixed LaTeX compilation: converted `app_logit_race_mfx.tex` from tinytable (`tblr`) to booktabs (`tabular`); stripped nested `\begin{table}` wrappers from all 8 R-generated table files; fixed `\m{1}` → `\mathbb{1}`; installed missing `titling` package; fixed `zelzer2020` typo; reformatted `quad_comparison.tex` headers to fit page width
- Updated `2_logit_twfe.R` to use `output = "kableExtra"` explicitly (prevents tinytable output)
- Both `draft-paper.tex` and `appendix.tex` compile cleanly
