...
- Added `0-setup.R` (renv activation + all `library()` calls), replaced `pacman::p_load()` in both orchestrators. Initialized renv (`renv.lock` tracked, `.Rprofile` gitignored).
- Memory management for cardioem/derm OOM: freed `final_ref_big` + `df_full_referrals` before quartet construction, year-by-year quartet loop with `gc()` between years, grid chunking in `make_block`/`make_block_win` (500K row chunks, identical results).
- Fixed `2_referrals_initial.R` mover detection range (2013:2018, was 2010:2018) to match MDPPAS 2012:2018 load. Replaced all `|>` with `%>%`.
- **Next**: restart R, re-run full pipeline, review cross-specialty MFX, then referee comments (3a → 4a → 1a/b → 3b).
- **TODO: check with ResDAC** on DUA status for RIF 2008-2012 carrier files.
- **TODO**: add ortho 2009-2018 robustness entry (`ortho_robust`) after main run.
