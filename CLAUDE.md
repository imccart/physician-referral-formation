...
- Added LaTeX intermediates and `*.pdf` to `.gitignore` (aligned with `/kickoff` template)
- Fixed table formatting: added `linesep = ""` to all `kable()` calls (prevents kableExtra's default `\addlinespace` every 5 rows); added `\midrule` separators between coefficient, FE indicator, and observation sections in footer rows
