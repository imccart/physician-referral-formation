...
- Cleaned up output files: removed unused `mfx_window.rds` (and its `saveRDS()` call), deleted orphaned `welfare_summary_quadruple.tex`
- Audited all output files — confirmed every remaining file in `results/` is referenced by paper or appendix
- Traced 4 static appendix tables to `physician-race` repo (`3_assess.R`)
- **Still open**: dynamics expansion, consequences reframe, heterogeneity analysis, medical school footnote (#13)
