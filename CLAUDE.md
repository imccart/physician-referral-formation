...
- Internal consistency review: fixed dynamics text (0.6→0.5, dropped specialists description), standardized "practice group" terminology across tables, updated date, removed "Preliminary and Incomplete".
- Table formatting: proper `$-$` minus signs in all R-generated tables, scientific notation for convergence FE SDs, reordered desc.tex rows logically, formatted integer counts.
- Added pipeline counts to `inline_stats.csv` (PCPs, specialists, movers, pairs).
- Paper (28pp) and appendix (16pp) compile cleanly.
- **Next steps**:
