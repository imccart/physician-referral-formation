...
- Planned response to internal referee report (`results/refreport_202602.md`), comment by comment.
- Established sequencing: Comment 2 (additional specialties) is highest priority but blocked on VRDC code export. Comments 1/3/4 can proceed on ortho-only in the meantime.
- Updated `/referee` skill to output .md instead of .tex; converted existing report.
...
- **Once code arrives**:
  1. Identify specialty/DRG codes for cardiology + dermatology from literature. Rationale for these three: ortho ~98% male specialists, cardiology ~85% male, dermatology ~50% female — spans the gender spectrum.
  2. Build `ReferralPairs_Cardiology.csv` and `ReferralPairs_Dermatology.csv` with same column structure as `ReferralPairs_Large.csv`. Place in `data/input/referrals/`.
  3. Parameterize `_BuildData.R` and `_main.R` to loop over specialties. Output files get specialty suffix (e.g., `df_logit_movers_cardiology.csv`).
  4. Run Jochmans + MFX + dynamics for each specialty.
