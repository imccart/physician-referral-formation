...
- Filled in `data.md`, `analysis.md`, `present.md` (were placeholders)
- Paper read-through: fixed "state" → "HRR", stale median, repetitive appendix references, wrong table footnote
- Removed all em dashes from paper text; documented no-em-dash style preference
- Added `inline_stats.csv` output to `1_descriptive_stats.R` so hardcoded paper numbers are verifiable
- **Next steps**:
