...
Unknown origin (TODO: track down source):
...
- Fixed Table 2 column headers: `\\` in kable col.names was rendered as `\textbackslash{}`. Fixed with `escape = FALSE` + `\shortstack[r]{...}` in R script and .tex file.
- Fixed Table 4 alignment: last column was `l` instead of `r` in `link_stats_by_window.tex` (static file, no generating script found).
- Added TODO to track down origin of `link_stats_by_window.tex`.
- **Next steps**:
  - Identify source of `link_stats_by_window.tex`
  - Run `/referee` skill for full paper review
  - Direct quality regression (does homophily predict lower specialist quality?)
